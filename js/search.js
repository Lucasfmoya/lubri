/* =============================================
   LUBRICENTRO O'HIGGINS — search.js v4
   ============================================= */
...
function render(data, patente) {
  ...
  contenedor.innerHTML = `
    <div class="hst-results-wrap fade-in">
      ${header}
      ${renderCard(reciente, patente, true)}
      ${acordeones}
      ${btnReporte}
    </div>`;
...
function scrollToResultados() {
  const seccion = document.querySelector(".bg-light");
...
(function init() {
  ...
  // Poblar input y disparar búsqueda
  if (input) {
    input.value = patente;
